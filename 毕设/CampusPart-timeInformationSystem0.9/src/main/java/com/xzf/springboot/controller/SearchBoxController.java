...
public class SearchBoxController {
   ...
   //管理员搜索
    ...
    //举报搜索
    @RequestMapping("/sys/reportSearch")
    public String queryreportList(@RequestParam(required=false,name ="start") String start,
                                  @RequestParam(required=false,name ="end") String end,
                                  @RequestParam(required=false,name ="state") String state,
                                  Model model,@RequestParam(defaultValue = "1",value = "pageNum")int pageNum){
        PageHelper.startPage(pageNum,5);
        List<Report> reports = reportService.searchReport(start, end,state);
