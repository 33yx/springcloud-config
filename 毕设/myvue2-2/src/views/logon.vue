  ...
  <div id="background">
    ...
    <h6>请如实填写</h6>
       ...
       <div class="form">
          <label for="">性别：</label>
          <label for="">男</label>
          <input type="radio" v-model.trim="sex" value="男">
          <label for="">女</label>
          <input type="radio" v-model.trim="sex" value="女">
        </div>
       <div class="form">
    ...
    <div v-if="identity==2" class="form">
      <label>营业执照号：</label><input type="text" v-model.trim="sbus"><br/>
    </div>
...
export default {
  ...
  data(){
    ...
    //   }
        ...
        if (this.identity==2){
          if (this.code==this.Emsg){
            axios.post(this.api.url+'/fore/addSellor'+
              '?ssname='+this.ssname+"&useage="+this.age+"&sbus="+this.sbus+"&address="+this.adress+"&uemail="+this.mail+"&uname="+this.relname
              +"&cardid="+this.caidid+"&phone="+this.tel+"&password="+this.password+"&usex="+this.sex+"&code="+this.Emsg,
            ).then(function (response)  {
              if(response.data.rid==1){
                console.log('数据接收');
                alert("注册成功，快去登陆吧");
